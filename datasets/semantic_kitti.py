...
class SemanticKITTIInternal:
    ...
    def return_double_views(self, index):
        ...
        #######################
        block_1 = block_.copy()
        theta = np.random.uniform(0, 2 * np.pi) # 旋转角度（0-2π）
        scale_factor = np.random.uniform(0.95, 1.05) # 缩放因子（0.95-1.05）
        rot_mat = np.array([[np.cos(theta), np.sin(theta), 0], # 旋转矩阵，对点云进行旋转
                            [-np.sin(theta),
                             np.cos(theta), 0], [0, 0, 1]])
        block_1[:, :3] = np.dot(block_1[:, :3], rot_mat) * scale_factor # 旋转和缩放（绕z轴旋转）
        # voxelization 将点云转换为体素网格
        ...
        if np.random.random() < 0.5:
            ...
            idxes = np.random.choice(idxes,
                                     int(ratio * block_.shape[0]),
                                     replace=False)
            block_2 = block_2[idxes]
            labels_2 = labels_2[idxes]
            ids_2_raw = ids_2_raw[idxes]

        # aug2: add noise (background clutter, label=255, id=-1)
        if np.random.random() < 0.5:
            ...
            noise_x = np.random.choice(np.arange(xmin, xmax, 0.01),
                                       noise_num,
                                       replace=True).astype(np.float32)
            noise_y = np.random.choice(np.arange(ymin, ymax, 0.01),
                                       noise_num,
                                       replace=True).astype(np.float32)
            noise_z = np.random.choice(np.arange(zmin, zmax, 0.01),
                                       noise_num,
                                       replace=True).astype(np.float32)
            noise_i = np.random.normal(loc=(imin + imax) / 2,
                                       scale=0.5,
                                       size=noise_num).astype(np.float32)
            noise = np.stack((noise_x, noise_y, noise_z, noise_i), axis=1)
            block_2 = np.concatenate((block_2, noise), axis=0)
            labels_2 = np.concatenate(
                (labels_2,
                 np.ones(noise.shape[0], dtype=np.int64) * 255),
                axis=0)
            ids_2_raw = np.concatenate(
                (ids_2_raw,
                 np.ones(noise.shape[0], dtype=np.int64) * (-1)),
                axis=0)
        ...
        if np.random.rand() < 0.5:
            block_2[:, 1] *= -1
        # aug6: random jittering (mild)
        if np.random.rand() < 0.5:
            jiterring = np.random.normal(
                loc=0., scale=0.01, size=(block_2.shape[0], 3))
            jiterring = np.clip(jiterring, a_min=-0.05, a_max=0.05)
            block_2[:, :3] += jiterring

        # 2) 在 base 强增强之后，再加一个小概率、温和版的 ASJ（原地旋转一个扇区）
        sj_prob = 0.3
        if np.random.rand() < sj_prob and block_2.shape[0] > 0:
            yaw = -np.arctan2(block_2[:, 1], block_2[:, 0])
            # 更窄的扇区：pi/3，避免过激
            azimuth_range = np.pi / 3.0
            start_angle = (np.random.random() * 2.0 - 1.0) * np.pi  # [-pi, pi)
            end_angle = start_angle + azimuth_range
            # wrap 到 [-pi, pi)
            end_angle = (end_angle + np.pi) % (2 * np.pi) - np.pi

            if start_angle < end_angle:
                sector = (yaw >= start_angle) & (yaw <= end_angle)
            else:
                # wrap-around 情况
                sector = (yaw >= start_angle) | (yaw <= end_angle)

            if sector.any():
                # 对该扇区内点做一个小角度旋转，模拟局部 weather/遮挡扰动
                delta_theta = (np.random.rand() - 0.5) * (np.pi / 6.0)  # [-pi/12, pi/12]
                rot_sj = np.array([[np.cos(delta_theta), np.sin(delta_theta), 0],
                                   [-np.sin(delta_theta),
                                    np.cos(delta_theta), 0], [0, 0, 1]])
                pts = block_2[sector, :3]
                block_2[sector, :3] = np.dot(pts, rot_sj)
        feat_2_ = block_2
