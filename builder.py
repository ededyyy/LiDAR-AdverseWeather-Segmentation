...
def make_model() -> nn.Module:
    if configs.model.name == 'minkunet':
        ...
        if 'cr' in configs.model:
            ...
            cr = 1.0
        # Optional: Learnable Point Drop (LPD) hyper-parameters.
        lpd_kwargs = {}
        if 'learnable_drop' in configs.model:
            lpd_kwargs['learnable_drop'] = bool(configs.model.learnable_drop)
        if 'lpd' in configs.model:
            # Allow nested config: model.lpd.*
            for k, v in configs.model.lpd.items():
                lpd_kwargs[k] = v
        model = MinkUNet(num_classes=configs.data.num_classes, cr=cr, **lpd_kwargs)
    else:
